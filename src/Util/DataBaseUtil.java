...
public class DataBaseUtil {
    ...
    public static boolean checkReader(String account, String password) {
        ...
        try {
            ...
            con = DriverManager.getConnection("jdbc:mysql://127.0.0.1:3306/rainmoon?useSSL=true&allowPublicKeyRetrieval=true&serverTimezone=UTC", "root", "Ls123456"); //链接本地MYSQL
            Statement stmt; //创建声明
    ...
    private static boolean isAlreadyBorrowed(String readerId, String bookId) {
        try {
            ...
            stmt = con.createStatement();
            //继续借书
            String selectSql = "select * from borrow where reader_id='"+readerId+"' and book_id='"+bookId+"'and is_back=0";
    ...
    public static Boolean addNewReader(Reader reader) {
        try {
            Connection con = null; //定义一个MYSQL链接对象
            Class.forName("com.mysql.cj.jdbc.Driver").newInstance();//MYSQL驱动
            con = DriverManager.getConnection("jdbc:mysql://127.0.0.1:3306/rainmoon?useSSL=false&allowPublicKeyRetrieval=true&serverTimezone=UTC", "root", "Ls123456"); //链接本地MYSQL
            Statement stmt; //创建声明
            stmt = con.createStatement();

            String updateSql = "insert into reader (Id,name,pass,type,sex,max_num,days_num,forfeit,email) values ('" + reader.getId() + "','" + reader.getName() + "','" + reader.getPassword() + "','"+ reader.getType()+"','" + reader.getSex() + "','" + reader.getMax_num()+ "','" + reader.getDays_num()+ "','" + reader.getForfeit()+ "','" + reader.getEmail() + "')";
            System.out.println(updateSql);
            int selectRes = stmt.executeUpdate(updateSql);
            if (selectRes != 0) {
                return true;
...
//                return true;
    ...
    public static String getMaxUserId() {
        ...
        try {
            ...
            String selectSql = "SELECT MAX(Id) AS max_id FROM reader"; // 获取最大ID的SQL语句
            ResultSet selectRes = stmt.executeQuery(selectSql); // 执行查询
            if (selectRes.next()) {
                maxId = selectRes.getString("max_id"); // 获取最大ID值
            }
            System.out.println(maxId);
            con.close(); // 关闭数据库连接
        } catch (Exception e) {
            System.out.print("获取最大用户ID时发生错误：" + e.getMessage());
        }
        return maxId;
    }
